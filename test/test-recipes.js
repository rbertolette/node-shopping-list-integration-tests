...
describe("Recipes", function() {
  ...
  //   EDGE CASE TESTS
  //   3. check that ingredients is an array
  //   4. check that there is at least one ingredient
...
it("should add an recipe on POST", function() {
  const newItem = { name: "granola", ingredients: ["oats", "raisins", "honey"]};
  return chai
    .request(app)
    .post("/recipes")
    .send(newItem)
    .then(function(res) {
      ...
      // solution included:
      expect(res.body.ingredients).should.include.members(newItem.ingredients);
    });
});
